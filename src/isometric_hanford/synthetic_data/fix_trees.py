...
# Placeholders for target tree style images
DEFAULT_TREE_REFERENCES = ["style_c.png", "style_d.png"]


def fix_trees(tile_dir: Path):
  load_dotenv()
  ...
  # Add tree references
  trees_dir = references_dir / "trees"
  tree_reference_names = [f.name for f in trees_dir.glob("*.png")]

  print(f"Found {len(tree_reference_names)} tree reference images in {trees_dir}")
  print(f"Tree reference images: {tree_reference_names}")

  tree_ref_ids = []
  for i, ref_name in enumerate(tree_reference_names):
    ref_path = trees_dir / ref_name
    if not ref_path.exists():
      print(f"Warning: Reference {ref_name} not found in {trees_dir}")
      continue
...
Edit the trees / green pixels in {images.get_index("generation")} to match the style of the other reference images.

Please remove extra details from the trees / green pixels in {images.get_index("generation")} - **reducing the level of detail** from the original generation and ensuring that there are more crisp grid pixels.

EVERYTHING ELSE MUST REMAIN THE SAME. DO NOT ADD ANY NEW TREES. Only fix the existing trees / green pixels in {images.get_index("generation")}.
...
def main():
  parser = argparse.ArgumentParser(
    description="Regenerate isometric pixel art tile with specific tree styling."
  )
  parser.add_argument(
    "tile_dir",
